...
void* softTriggerProc(void* pUserData)
{
	int ret = IMV_OK;
	IMV_HANDLE devHandle = (IMV_HANDLE)pUserData;
	if (NULL == devHandle)
...
int main()
{
	...
	pthread_t frameThreadHandle = 0;
	pthread_t triggerThreadHandle = 0;
	...
	do
	{
		...
		if (pthread_create(&frameThreadHandle, 0, frameGrabbingProc, (void*)devHandle) != 0)
		{
			printf("Failed to create frame grabbing thread!\n");
			break;
		}

		// 创建软触发线程 
		// Create soft Trigger thread
		if (pthread_create(&triggerThreadHandle, 0, softTriggerProc, (void*)devHandle) != 0)
		{
			g_isExitThread = true;
		...
		pthread_join(triggerThreadHandle, NULL);

		pthread_join(frameThreadHandle, NULL);
