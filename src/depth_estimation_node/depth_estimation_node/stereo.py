...
def rectify_images(img1, img2, camera_matrix, dist_coeffs, cam1_ext, cam2_ext, show=True, out_prefix='images/rectified'):
	#Runs stereoRectify and then initUndistortRectifyMap + remap to produce rectified images
...
def match_sparse(img1_rect, img2_rect, detector, max_matches=500):
	...
	bf = cv2.BFMatcher(cv2.NORM_HAMMING, crossCheck=True, ) #might want to try FLANN here
...
def gen_pointcloud_from_params(img1, img2, camera_matrix, dist_coeffs, cam1_ext, cam2_ext):
	...
	try:
		if camera_matrix is not None and cam1_ext is not None and cam2_ext is not None:
			img1_rect, img2_rect, P1, P2, Q = rectify_images(img1, img2, camera_matrix, dist_coeffs, cam1_ext, cam2_ext)
		else:
